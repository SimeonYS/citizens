...
class CitizensPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `citizens`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM citizens WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO citizens (date, title, link, content)"
